    ...
    methods: {
        async loadTrackDetail() {
            try {
                ...
                const res = await trackApi.getTrackDetail(this.trackId);
        ...
        async exportTrack() {
            ...
            try {
                const result = await new Promise((resolve, reject) => {
                    uni.showActionSheet({
                        itemList: formatOptions.map(option => option.label),
                        success: (res) => {
                            resolve(formatOptions[res.tapIndex].value);
                        },
                        fail: (err) => {
                            reject(err);
                        }
                ...
                const confirmResult = await new Promise((resolve) => {
                    uni.showModal({
                        title: '确认导出',
                        content: `确定要导出为${this.getFormatName(result)}格式吗？`,
                        success: (res) => {
                            resolve(res.confirm);
                        }
                    });
        ...
        async doExport(format) {
            try {
                ...
                const response = await trackApi.exportTrack(this.trackId, format);
                const trackName = this.getTrackName();
                const fileName = `${trackName}.${this.getFileExtension(format)}`;

                await this.saveFile(response, fileName);
                uni.hideLoading();
        ...
        async saveFile(uint8ArrayData, fileName) {
            try {
                const platform = uni.getSystemInfoSync().platform;

                if (platform === 'h5') {
                    this.saveFileH5(uint8ArrayData, fileName);
                } else {
                    this.saveFileNative(uint8ArrayData, fileName);
                }
                ...
                uni.showModal({
                    title: '导出成功',
                    content: '轨迹数据已导出完成，请检查下载文件夹',
                    showCancel: false
                });
            }
        },

        saveFileH5(uint8ArrayData, fileName) {
            try {
                let binary = '';
        ...
        saveFileNative(uint8ArrayData, fileName) {
            try {
                this.downloadFileMobile(uint8ArrayData, fileName);
            } catch (error) {
                console.error('原生环境保存文件失败:', error);
                uni.showModal({
                    title: '导出成功',
                    content: '轨迹数据已导出完成，文件已保存到手机存储',
                    showCancel: false
                });
            }
        },

        downloadFileMobile(uint8ArrayData, fileName) {
            try {
                this.saveToDownloadFolder(uint8ArrayData, fileName);
            } catch (error) {
                console.error('手机端下载失败:', error);
                this.saveToDevice(uint8ArrayData, fileName);
            }
        },

        saveToDownloadFolder(uint8ArrayData, fileName) {
            try {
                // #ifdef APP-PLUS
                let osName = plus.os.name;
                if (osName === 'Android') {
                    this.saveToAndroidDownload(uint8ArrayData, fileName);
                } else {
                    this.saveToIOS(uint8ArrayData, fileName);
                }
                // #endif
                // #ifndef APP-PLUS
                this.saveToDevice(uint8ArrayData, fileName);
                ...
                console.error('保存到Download目录失败:', error);
                this.saveToDevice(uint8ArrayData, fileName);
            }
        },

        saveToAndroidDownload(uint8ArrayData, fileName) {
            plus.android.requestPermissions([
                'android.permission.WRITE_EXTERNAL_STORAGE',
                'android.permission.READ_EXTERNAL_STORAGE'
            ], success => {
                this.startAndroidDownload(uint8ArrayData, fileName);
            }, error => {
                console.error('权限获取失败:', error);
                uni.showToast({
                    title: '无法获取存储权限，文件下载将出错',
                    icon: 'none'
                });
                this.saveToDevice(uint8ArrayData, fileName);
            });
        ...
        startAndroidDownload(uint8ArrayData, fileName) {
            try {
                uni.showLoading({
                    title: '正在保存'
                });
                ...
                plus.io.resolveLocalFileSystemURL(saveDir, (dirEntry) => {
                    ...
                    plus.io.resolveLocalFileSystemURL(privateDir, (rootEntry) => {
                        rootEntry.getDirectory('轨迹导出', { create: true }, (dirEntry) => {
                            dirEntry.getFile(fileName, { create: true }, (fileEntry) => {
                                fileEntry.createWriter((writer) => {
                                    writer.onwrite = (evt) => {
                                        uni.hideLoading();
        ...
        saveToIOS(uint8ArrayData, fileName) {
            try {
                uni.showLoading({
                    title: '正在保存'
                });
        ...
        saveToDevice(uint8ArrayData, fileName) {
            try {
                let binary = '';
                const bytes = new Uint8Array(uint8ArrayData);
                const len = bytes.byteLength;
        ...
        downloadFileDirect(uint8ArrayData, fileName) {
            try {
