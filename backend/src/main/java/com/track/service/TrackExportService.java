...
/**
 * 轨迹导出服务 (流式输出版)
 * 优化点：不再将整个文件生成到内存(byte[])，而是直接写入 Response 的 OutputStream，防止大文件 OOM。
 */
@Service
public class TrackExportService {
     ...
     * 导出为GPX格式 (流式)
     */
    public void exportToGpxStream(TrackDetail trackDetail, OutputStream out) throws IOException {
        // 使用 BufferedWriter 包装，提高写入性能
     ...
     * 导出为KML格式 (流式)
     */
    public void exportToKmlStream(TrackDetail trackDetail, OutputStream out) throws IOException {
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(out, StandardCharsets.UTF_8))) {
     ...
     * 包含 BOM 头处理
     */
    public void exportToCsvStream(TrackDetail trackDetail, OutputStream out) throws IOException {
        
        // 【关键点】BOM 是字节，必须在创建字符流 Writer 之前写入
        // 如果包在 OutputStreamWriter 里写，会被当做字符编码处理，导致文件头损坏
        ...
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(out, StandardCharsets.UTF_8))) {
            ...
            if (points != null) {
                for (int i = 0; i < points.size(); i++) {
                    ...
                    // 使用 StringBuilder 拼接这一行，减少 I/O 调用次数
                    StringBuilder sb = new StringBuilder();
                    sb.append(i + 1).append(",");
                    sb.append(getSafeString(point.getLongitude(), "")).append(",");
                    sb.append(getSafeString(point.getLatitude(), "")).append(",");
                    sb.append(getSafeString(point.getAltitude(), "")).append(",");
                    sb.append(getSafeString(point.getSpeed(), "")).append(",");
                    sb.append(getSafeString(point.getSatelliteCount(), "")).append(",");
                    sb.append(escapeCsv(point.getAddress())).append(",");
                    sb.append(formatDateTime(point.getCreateTime()));
                    sb.append("\n");
                    
                    writer.write(sb.toString());
                    
                    // 可选：每1000行 flush 一次，防止缓冲区积压（BufferedWriter 默认也会处理）
                    if (i % 1000 == 0) {
                        writer.flush();
                    }
                }
     ...
     * 导出为GeoJSON格式 (流式)
     */
    public void exportToGeoJsonStream(TrackDetail trackDetail, OutputStream out) throws IOException {
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(out, StandardCharsets.UTF_8))) {
